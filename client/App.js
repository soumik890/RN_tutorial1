...
const App=()=>{
  ...
  const [editText, setEditText] = useState(todo.info)
...
{/* <ScrollView>
  ...
  {!edit?<Text style={style.txt}>{todo.info||todo.text}, {todo.id}</Text>:<TextInput style={style.inp} value={editText} onChangeText={(text)=>setEditText(text)}/>}
  <TouchableOpacity> 
  <AntDesign name="checkcircle" size={24} color="black"/>
  </TouchableOpacity>
  <TouchableOpacity> 
  <AntDesign style={style.icon} name="delete" size={24} color="black" onPress={()=>deletetodo(todo.id)}/>
  </TouchableOpacity>
  </View>
   // <Text key={todo.id} style={style.action}>{todo.info}</Text>
   ))}
</ScrollView> */}
